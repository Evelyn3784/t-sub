// static/app.js
// UI-логика управления приложением.
// Комментарии на русском.
// Поддерживает WebSocket push (логи и статус), загрузку .session и все кнопки управления.
...
(function () {
  ...
  const enableScheduledCheckEl = document.getElementById("enable_scheduled_check");
  ...
  // Устанавливаем соединение WebSocket для логов
  function connectLogsWS() {
    const proto = (location.protocol === "https:") ? "wss" : "ws";
    const url = `${proto}://${location.host}${WS_LOGS}`;
    try {
    ...
    wsLogs.onmessage = (ev) => {
      // Первое сообщение может быть JSON init
      try {
  ...
  // WebSocket для статуса/push
  function connectStatusWS() {
    const proto = (location.protocol === "https:") ? "wss" : "ws";
    const url = `${proto}://${location.host}${WS_STATUS}`;
    try {
    ...
    wsStatus.onmessage = (ev) => {
      try {
        ...
        if (data && data.stats) {
          // можно отобразить статистику, но сейчас обновляем accounts
        }
        if (data && data.accounts_meta) {
          updateAccounts(data.accounts_meta, data.cooldowns || {});
        }
        // refresh progress if initial push
        if (data && data.stats) {
          refreshProgressOnce();
        }
      } catch (e) {
  ...
  function updateAccounts(accounts_meta, cooldowns) {
    ...
    for (const name of names) {
      ...
      tr.innerHTML = `<td>${escapeHtml(name)}</td><td>${auth}</td><td>${cooldown_display}</td><td>${escapeHtml(meta.last_action||"")}</td>`;
  ...
  // Progress table: один раз загрузить
    ...
    fetch("/api/list_uploaded_sessions").then(r => r.json()).then(j => {
      ...
      for (const s of arr) {
        ...
        tr.innerHTML = `<td>${escapeHtml(s.filename)}</td><td>${escapeHtml(s.name||"")}</td><td>${escapeHtml(s.api_id||"")}</td><td>${exists}</td>
          <td><button class="deleteSessionBtn" data-file="${encodeURIComponent(s.filename)}">Удалить</button></td>`;
  ...
  btnUpload.addEventListener("click", () => {
    ...
    if (!f) { alert("Выберите файл .session"); return; }
    const name = sessionNameInput.value.trim();
    const api_id = sessionApiIdInput.value.trim();
    const api_hash = sessionApiHashInput.value.trim();
    if (!name || !api_id || !api_hash) { alert("Заполните: имя, API_ID, API_HASH"); return; }
    const fd = new FormData();
    fd.append("file", f, f.name);
    fd.append("name", name);
    fd.append("api_id", api_id);
    fd.append("api_hash", api_hash);
    btnUpload.disabled = true; btnUpload.textContent = "Загрузка...";
    fetch("/api/upload_session", { method: "POST", body: fd }).then(r=>r.json()).then(j=>{
      btnUpload.disabled = false; btnUpload.textContent = "Загрузить .session";
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else {
        appendLog("UI: файл сессии загружен: " + (j.file || f.name));
        refreshUploadedSessions();
      }
    }).catch(e=>{
      btnUpload.disabled = false; btnUpload.textContent = "Загрузить .session";
  ...
  // Кнопки: старт/пауза/резюме/стоп
  btnStart.addEventListener("click", () => {
    ...
    if (isNaN(min) || isNaN(max) || min < 0 || max < min) { alert("Проверьте задержки min/max"); return; }
    const fd = new FormData();
    ...
    fd.append("membership_check_enabled", (enableScheduledCheckEl && enableScheduledCheckEl.checked) ? "1" : "0");
    btnStart.disabled = true; btnStart.textContent = "Запускаю...";
    fetch("/api/start", { method: "POST", body: fd }).then(r=>r.json()).then(j=>{
      btnStart.disabled = false; btnStart.textContent = "Запустить Подписку";
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else {
  ...
  btnStop.addEventListener("click", ()=> {
    ...
    fetch("/api/stop", {method:"POST"}).then(r=>r.json()).then(j=>{
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else appendLog("UI: запрошена остановка задачи подписки.");
    }).catch(e=>alert("Ошибка: " + e));
  });
  ...
  btnCheckNow.addEventListener("click", ()=> {
    fetch("/api/check_now", {method:"POST"}).then(r=>r.json()).then(j=>{
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else { appendLog("UI: запущена немедленная проверка."); progressLoaded = false; }
    }).catch(e=>alert("Ошибка: " + e));
  });
  ...
  btnUnsub.addEventListener("click", ()=> {
    ...
    fetch("/api/unsubscribe", {method:"POST"}).then(r=>r.json()).then(j=>{
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else appendLog("UI: задача отписки запущена.");
    }).catch(e=>alert("Ошибка: " + e));
  });
  ...
  btnClearGood.addEventListener("click", ()=> {
    ...
    fetch("/api/clear_good_targets", { method: "POST", body: fd }).then(r=>r.json()).then(j=>{
      if (!j.ok) alert("Ошибка: " + (j.msg || JSON.stringify(j)));
      else appendLog("UI: good_targets.csv очищен.");
    }).catch(e=>alert("Ошибка: " + e));
  });
